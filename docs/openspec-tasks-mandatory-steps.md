...
- **Step N+3**: E2E Testing with agent-browser (MANDATORY if applicable) - **AGENT MUST EXECUTE**
...
### Step N+3: E2E Testing with agent-browser (MANDATORY if applicable)

**Agent Responsibility**: The coding agent MUST execute all E2E tests using `agent-browser`. This is NOT optional and cannot be delegated to the user.
...
2. **Navigate to Application**:
   - Use `agent-browser open` to open the application URL
   - Wait for page to load completely
   - Use `agent-browser snapshot` to verify initial state

3. **Execute User Workflows**:
   - Use `agent-browser` commands to interact with the UI:
     - `agent-browser click` for button clicks and navigation
     - `agent-browser fill` or `agent-browser keyboard type` for form inputs
     - `agent-browser snapshot` to verify state changes
     - `agent-browser wait` for async operations
...
## 11. Frontend: E2E Testing with agent-browser (MANDATORY if applicable - AGENT MUST EXECUTE)
- [ ] 11.1 Ensure frontend and backend servers are running
- [ ] 11.2 Navigate to application using `agent-browser open`
- [ ] 11.3 Execute complete user workflow using `agent-browser` commands
...
1. **Execute All Manual Tests**: Never ask the user to run curl commands or E2E tests. The agent must:
   ...
   - Execute all E2E tests using `agent-browser`
